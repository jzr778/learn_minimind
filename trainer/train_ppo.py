...
def ppo_train_epoch(epoch, loader, iters, old_actor_model, ref_model, actor_scheduler, critic_scheduler, reward_model, reward_tokenizer, start_step=0, wandb=None):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        prompt_length = enc.input_ids.shape[1] # 标记prompt长度
        ...
        with torch.no_grad():
            ...
            # 用当前的 actor（策略模型）对 prompt 进行“采样式生成”，形状 [prompt | response | pad...]
            gen_out = model_for_gen.generate(
                input_ids=enc.input_ids, attention_mask=enc.attention_mask,
                max_new_tokens=args.max_gen_len, do_sample=True, temperature=0.8,
                pad_token_id=tokenizer.pad_token_id, eos_token_id=tokenizer.eos_token_id)  # [B, P+R+pad]

        responses_text = [tokenizer.decode(gen_out[i, prompt_length:], skip_special_tokens=True) for i in range(len(prompts))] # [B, R+pad]
        # Q(s,a)，即Q(prompts, responses) -> rewards
        ...
        last_indices = (full_mask * torch.arange(full_mask.size(1), device=gen_out.device)).argmax(dim=1) # [B] 每一行最后一个非padding位置的index
        ...
        # 计算整条 response 的 log概率
        actor_logp = (logp_tokens * final_mask).sum(dim=1)  # [B]

        with torch.no_grad(): # old_actor和ref_model只用来打分
            old_logits = old_actor_model(input_ids=gen_out, attention_mask=full_mask).logits  # [B, P+R+pad, V]
            old_logp_tokens = F.log_softmax(old_logits[:, :-1], dim=-1).gather(2, labels.unsqueeze(-1)).squeeze(-1)  # [B, P+R+pad-1]
            old_logp = (old_logp_tokens * final_mask).sum(dim=1)  # [B]
            
            ref_logits = ref_model(input_ids=gen_out, attention_mask=full_mask).logits  # [B, P+R+pad, V]
            ref_logp_tokens = F.log_softmax(ref_logits[:, :-1], dim=-1).gather(2, labels.unsqueeze(-1)).squeeze(-1)  # [B, P+R+pad-1]
